...
def _getFeatures(root):
    ...
    for event, elem in etree.iterwalk(root, events=("start", "end")):
        ...
        if tag in __conditionals_elif and event == 'end' and ns == __cppnscpp:
            parcon = False
...
def apply(folder):
    ...
    annotations2andmore = list(map(lambda s: set(s), list(annotationmap.values())))
...
def addCommandLineOptionsMain(optionparser):
    ...
    optionparser.add_argument("--folder", dest="folder", help="input folder [default: %(default)s]", default=".")
