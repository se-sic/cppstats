...
function cppstats_general_check() {
	...
	./pxml.py --folder "${1}/_cppstats" ${ARGS} 2>&1 | tee -a `printf ${LOGFILE} ${FOLDER}` >> /dev/null
...
export LOGFILE
export ARGS
export -f cppstats_general_check


parallel --gnu --no-notice --arg-file "${INPUTFILE}" cppstats_general_check
